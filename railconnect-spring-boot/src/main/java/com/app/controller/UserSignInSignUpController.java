...
public class UserSignInSignUpController {
	...
	// sign up
	@PostMapping("/signup")
	public ResponseEntity<?> userSignup(@RequestBody @Valid UserEntity dto) {
	...
	public ResponseEntity<?> signinUser(@RequestBody @Valid SigninRequest reqDTO) {
		...
		// => auth success
		UserDetailDTO user = userService.getUserDetailsByEmail(reqDTO.getEmail());
		return ResponseEntity
				.ok(new SigninResponse(user.getId(),utils.generateJwtToken(verifiedAuth), "Successful Authentication!!!"));
